...
@Component({
  selector: 'app-check-order-item',
  templateUrl: './check-order-product-item.component.html',
  styleUrls: ['./check-order-product-item.component.scss'],
})
export class CheckOrderProductItemComponent implements OnInit {
    ...
    if (this.p.length < this.productList.length) {
      for (let i = this.p.length; i < this.productList.length; i++) {
        this.p.push(this._formBuilder.group({
          ...
          itemPickedQ: [{value: this.productList[i].pickedQ, disabled: true}],
